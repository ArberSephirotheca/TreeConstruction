...
public:
    Pipe(AppParams param, AAssetManager* assetManager) : ApplicationBase(assetManager) {
        params_ = param;
    }
    void allocate();
    ~Pipe();
...
void Pipe::init(const int num_blocks, const int queue_idx){
    Init init_stage = Init(assetManager_);
...
void Pipe::unique(const int num_blocks, const int queue_idx){
    ...
    auto unique_stage = Unique(assetManager_);
...
void Pipe::radix_tree(const int num_blocks, const int queue_idx){
    ...
    auto radix_tree_stage = RadixTree(assetManager_);
...
void Pipe::edge_count(const int num_blocks, const int queue_idx){
    ...
    auto edge_count_stage = EdgeCount(assetManager_);
...
void Pipe::prefix_sum(const int num_blocks, const int queue_idx){
    ...
    auto prefix_sum_stage = DownSweepPrefixSum(assetManager_);
...
void Pipe::octree(const int num_blocks, const int queue_idx){
    ...
    auto build_octree_stage = Octree(assetManager_);

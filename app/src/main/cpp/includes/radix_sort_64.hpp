...
class RadixSort64 : public ApplicationBase{
    public:
    RadixSort64(AAssetManager* assetManager) : ApplicationBase(assetManager) {};
    ~RadixSort64() {};
...
//	vkQueueWaitIdle(singleton.queues[queue_idx]);
    auto result = vkWaitForFences(singleton.device, 1, &fence, VK_TRUE, UINT64_MAX);
    if (result != VK_SUCCESS){
        __android_log_print(ANDROID_LOG_ERROR , "Vulkan", "radix sort 64 failed to wait for fence");
        std::cout << "radix sort 64: Fails to wait for fence!"<<std::endl;
    }

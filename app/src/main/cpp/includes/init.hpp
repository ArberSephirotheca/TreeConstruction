...
class Init : public ApplicationBase{
    public:
    Init(AAssetManager* assetManager) : ApplicationBase(assetManager) {
        __android_log_print(ANDROID_LOG_DEBUG, "ApplicationBase", "Init AssetManager passed: %p", assetManager_);

//        __android_log_print(ANDROID_LOG_DEBUG, "Main", "Init Start to load shader");

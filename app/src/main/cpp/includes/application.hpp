...
class ApplicationBase{
    ...
    protected:
	   ...
	   // VkQueue queue;
        ...
        ApplicationBase(AAssetManager* assetManager): singleton(Singleton::get_singleton()) {
	        //create_compute_queue();
            if (assetManager == nullptr) {
                // Asset Manager is not available
                __android_log_print(ANDROID_LOG_ERROR, "JNI", "Failed to retrieve AAssetManager");
            }
            assetManager_ = assetManager;
            __android_log_print(ANDROID_LOG_DEBUG, "ApplicationBase", "AssetManager passed: %p", assetManager_);
...
VkPipelineShaderStageCreateInfo load_shader(const std::string shader_name, VkShaderModule *shaderModule){
    ...
    shaderStage.module = tools::loadShader((shadersPath + shader_name).c_str(), singleton.device, assetManager_);
